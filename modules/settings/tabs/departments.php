...
if (!empty($_GET['ajax'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'delete' && $id) {
            $st = $db->prepare('SELECT COUNT(*) FROM employees WHERE department_id = ?');
            $st->execute([$id]);
...
$(function () {
    ...
    if ($.fn.DataTable) $('#deptTable').DataTable({ pageLength: 25, order: [[1,'asc']], columnDefs: [{ orderable:false, targets:[0,4] }], language: { emptyTable: 'No departments defined.' } });
    ...
    $(document).on('click', '.btn-del-dept', function () {
        var b = $(this);
        if (!confirm('Delete department "' + b.data('name') + '"?')) return;
