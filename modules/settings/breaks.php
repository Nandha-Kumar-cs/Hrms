...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_times') {
        ...
        if (!$errors) {
            ...
            // Mirror onto the General shift — payroll subtracts the SHIFT's breaks
            // now, so without this these values would have no effect. Other shifts'
            // breaks are edited in Settings → Shifts.
            try {
                $db->prepare("UPDATE shifts SET lunch_start = ?, lunch_end = ? WHERE name = 'General'")
                   ->execute([$vals['lunch_start'], $vals['lunch_end']]);
            ...
            flash('success', 'Break times saved (applied to the General shift; other shifts are edited in Settings → Shifts).');
            redirect($self);
        }
    }
    ...
    if ($action === 'add_batch' || $action === 'update_batch') {
        ...
        if (!$errors) {
            if ($action === 'add_batch') {
                ...
                // Un-assign employees whose shift no longer matches this batch.
                if ($bShift !== null) {
    ...
    <div class="card page-card">
        ...
        <div class="card-body">
            <table class="table table-sm align-middle">
                <thead class="table-light"><tr><th>Batch</th><th>Shift</th><th>Start</th><th>End</th><th class="text-center">Duration</th><th class="text-center">Employees</th><th class="text-end">Action</th></tr></thead>
                <tbody>
                <?php if (!$batches): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <select name="shift_id" class="form-select form-select-sm">
                                ...
                                <option value="<?= (int)$so['id'] ?>" <?= (int)($b['shift_id'] ?? 0) === (int)$so['id'] ? 'selected' : '' ?>>
                                    <?= h($so['name']) ?>
                                </option>
                                <?php endforeach; ?>
                            </select>
