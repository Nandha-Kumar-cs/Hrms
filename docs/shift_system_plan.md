...
>
> - ✅ **Phase 5 — ROTATIONAL SHIFTS** — `employee_shift_schedule` (effective-dated: employee + shift + date range) lets someone be on Morning for 2 weeks, then General for 2 weeks. `employee_shift_on($empId, $date)` resolves the shift **as of the attendance date**, so back-dated marking and biometric imports are judged by the shift in force on the day worked — not whatever the employee is on today. New **Settings → Shift Rotation** screen generates a repeating cycle for many employees at once, plus manual blocks for swaps/cover. Migration: `install/add_shift_rotation.sql`.
>
> **ALL PHASES COMPLETE.** Verified end to end: 28/28 shift assertions + 16/16 rotation assertions, exact regression match on a stored slip, live mark POST, shift filters, and the deactivation guard.
>
